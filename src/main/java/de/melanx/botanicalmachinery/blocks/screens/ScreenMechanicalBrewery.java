...
public class ScreenMechanicalBrewery extends ScreenBase<ContainerMechanicalBrewery> {
    public ScreenMechanicalBrewery(ContainerMechanicalBrewery container) {
        super(container);
        this.ySize = 192;
        this.manaBar.y = 28;
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        if (tile.getInventory().getStackInSlot(0).isEmpty())
            RenderHelper.renderFadedItem(this, new ArrayList<>(TileMechanicalBrewery.BREW_CONTAINER), this.guiLeft + 44, this.guiTop + 48);
        if (tile.getProgress() > 0) {
            ...
            this.drawTexturedModalRect(this.guiLeft + 96, this.guiTop + 48,  176, 0, Math.round(22 * pct), 16);
        }
    }
}
